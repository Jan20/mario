...
export class World implements OnInit {
  ...
  public constructor(
    ...
    private levelService: LevelService    
      ...
      this.stage = new Stage(this, levelService.getLevel(this.levelIndex))
  ...
  public generateEnemies(): void {
      ...
      for (var i = 0; i < this.levelService.getLevel(this.levelIndex)[0].length; i++) {
          for (var j = 0; j < this.levelService.getLevel(this.levelIndex).length; j++) {
              // G = ground crawler
              // J = Jumper
              // A = aerial
              // Assume these are all placed smartly, too much unnecessary checking otherwise
              var stage = this.levelService.getLevel(this.levelIndex)
              ...
              if (currentSquare == 'C') {
                  ...
                  var xMin = 0;
                  var xMax = Math.ceil(this.levelService.getLevel(this.levelIndex)[0].length);
                  //min
                  for (var k = i - 1; k >= 0; k--)
                  ...
                  // max
                  for (var k = i + 1; k < Math.ceil(this.levelService.getLevel(this.levelIndex)[0].length); k++)
                      if (stage[j][k] != '.' || stage[j + 1][k] == '.') {
                          xMax = k - 1;
                          break;
                      }
